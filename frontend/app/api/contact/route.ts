...
export async function POST(request: Request) {
  try {
    ...
    if (recaptchaVerification.status === 'misconfigured') {
      ...
      return NextResponse.json(
        {error: 'The contact form is temporarily unavailable. Please call us at (314) 631-6738.'},
        {status: 503},
      )
    }
    ...
    if (recaptchaVerification.status === 'rejected') {
      return NextResponse.json(
        {error: 'Verification failed. Please try again, or call us at (314) 631-6738.'},
        {status: 400},
      )
    }
    ...
    await transporter.sendMail({
      ...
      html: `
        ...
        ${lines}
        <hr />
        <p style="color: #888; font-size: 12px;">Sent from the Kingdom Canine website contact form.</p>
      `,
    })
